<script lang="ts">
  ...
  function validateForm() {
    if (password !== confirmPassword) {
      error = t('auth.signup.passwordsNotMatch');
      return false;
    }
    if (password.length < 6) {
      error = t('auth.signup.passwordTooShort');
      return false;
    }
    return true;
  }
...
<div class="auth-container">
  <div class="auth-card">
      ...
      use:enhance={({ formElement }) => {
        ...
        return async ({ result, update }) => {
          ...
          if (result.type === 'redirect') {
            ...
            error = result.data?.error || t('auth.signup.registrationFailed');
          }
    ...
    <p>
      ...
      <a href="/auth/login">{t('auth.signup.signInHere')}</a>
    </p>
  </div>
</div>
